...
var checkboxTheme = {
  ...
  style: {
    ...
    keysHelpTip: (keys2) => keys2.map(([key, action]) => `${import_yoctocolors_cjs3.default.bold(key)} ${import_yoctocolors_cjs3.default.dim(action)}`).join(import_yoctocolors_cjs3.default.dim(" \u2022 "))
  },
  helpMode: "always",
  keybindings: []
};
...
function normalizeChoices(choices) {
  return choices.map((choice) => {
    ...
    const normalizedChoice = {
      ...
      checkedName: (_c = choice.checkedName) != null ? _c : name,
      disabled: (_d = choice.disabled) != null ? _d : false,
      checked: (_e = choice.checked) != null ? _e : false
    };
...
var esm_default2 = createPrompt((config, done) => {
  ...
  if (theme.helpMode !== "never" && instructions !== false) {
    if (typeof instructions === "string") {
      ...
      if (shortcuts.all)
        keys2.push([shortcuts.all, "all"]);
      if (shortcuts.invert)
        keys2.push([shortcuts.invert, "invert"]);
      keys2.push(["\u23CE", "submit"]);
      helpLine = theme.style.keysHelpTip(keys2);
    }
  }
  const lines = [
...
var ExternalEditor = class {
  ...
  createTemporaryFile() {
    var _a;
    try {
      const baseDir = (_a = this.fileOptions.dir) != null ? _a : import_node_os.default.tmpdir();
  ...
  readTemporaryFile() {
    ...
    try {
      ...
      if (tempFileBuffer.length === 0) {
        ...
        let encoding = (_a = (0, import_chardet.detect)(tempFileBuffer)) != null ? _a : "utf8";
  ...
  launchEditor() {
    ...
    try {
      ...
      this.lastExitStatus = (_a = editorProcess.status) != null ? _a : 0;
    } catch (launchError) {
...
var esm_default6 = createPrompt((config, done) => {
  ...
  const [status, setStatus] = useState("idle");
  const [value, setValue] = useState("");
  const validDefault = validateNumber(config.default, { min, max, step }) === true ? (_a = config.default) == null ? void 0 : _a.toString() : void 0;
  ...
  useKeypress(async (key, rl) => {
    ...
    if (isEnterKey(key)) {
      ...
      if (isValid === true) {
        setValue(String(answer != null ? answer : ""));
        setStatus("done");
        done(answer);
      } else {
        rl.write(value);
...
var esm_default7 = createPrompt((config, done) => {
  ...
  const [status, setStatus] = useState("idle");
  const [value, setValue] = useState("");
  const [expanded, setExpanded] = useState((_a = config.expanded) != null ? _a : false);
  const [errorMsg, setError] = useState();
  const theme = makeTheme(config.theme);
...
function normalizeChoices3(choices) {
  ...
  return choices.map((choice) => {
    ...
    return {
      ...
      name,
      short: (_b = choice.short) != null ? _b : name,
      key: (_c = choice.key) != null ? _c : String(index)
    };
  });
}
...
var searchTheme = {
  ...
  style: {
    ...
    keysHelpTip: (keys2) => keys2.map(([key, action]) => `${import_yoctocolors_cjs6.default.bold(key)} ${import_yoctocolors_cjs6.default.dim(action)}`).join(import_yoctocolors_cjs6.default.dim(" \u2022 "))
  },
  helpMode: "always"
};
...
var esm_default10 = createPrompt((config, done) => {
  ...
  const description = selectedChoice == null ? void 0 : selectedChoice.description;
  const header = [prefix, message, searchStr].filter(Boolean).join(" ").trimEnd();
  const body = [
    error != null ? error : page,
    " ",
    description ? theme.style.description(description) : "",
    helpLine
  ].filter(Boolean).join("\n").trimEnd();
...
var selectTheme = {
  ...
  style: {
    ...
    keysHelpTip: (keys2) => keys2.map(([key, action]) => `${import_yoctocolors_cjs7.default.bold(key)} ${import_yoctocolors_cjs7.default.dim(action)}`).join(import_yoctocolors_cjs7.default.dim(" \u2022 "))
  },
  helpMode: "always",
  indexMode: "hidden",
  keybindings: []
