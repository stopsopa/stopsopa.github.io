/**
 * from: https://github.com/ajaxorg/ace/blob/23208f2f19020d1f69b90bc3b02460bda8422072/demo/kitchen-sink/dev_util.js
 */
...
define("kitchen-sink/inline_editor", ["require", "exports", "module", "ace/line_widgets", "ace/editor", "ace/virtual_renderer", "ace/lib/dom", "ace/commands/default_commands"], function (
  require,
  exports,
  module
) {
...
define("ace/test/user", ["require", "exports", "module"], function (require, exports, module) {
  ...
  function updateModifierStates(keyCode) {
    if (keyCode == controlKeys.Shift) return (shift = true);
    if (keyCode == controlKeys.Control) return (ctrl = true);
    if (keyCode == controlKeys.Meta) return (meta = true);
    if (keyCode == controlKeys.Alt) return (alt = true);
  }
...
define("ace/test/asyncjs/assert", ["require", "exports", "module", "ace/lib/oop"], function (require, exports, module) {
  ...
  assert.AssertionError.prototype.toString = function () {
    if (this.message) {
      ...
      return [this.name + ":", toJSON(this.expected), this.operator, toJSON(this.actual)].join(" ");
    }
  };
...
define("ace/test/asyncjs/async", ["require", "exports", "module"], function (require, exports, module) {
  ...
  (function () {
        ...
        source.next(function handler(err, value) {
          if (err) callback(err);
            ...
            filter(value, function (err, takeIt) {
              if (err) callback(err);
              else if (takeIt) callback(null, value);
              else source.next(handler);
            });
          }
        });
      };
    ...
    this.slice = function (begin, end) {
      var count = -1;
      if (!end || end < 0) var end = Infinity;
      var source = this.source;
      this.next = function (callback) {
        source.next(function handler(err, value) {
          count++;
          if (err) callback(err);
          else if (count >= begin && count < end) callback(null, value);
          else if (count >= end) callback(STOP);
          else source.next(handler);
        });
      };
      return new this.constructor(this);
    ...
    this.reduce = function (reduce, initialValue) {
      ...
      var source = this.source;
      this.next = function (callback) {
        if (done) return callback(STOP);
        if (initialValue === undefined) {
          source.next(function (err, currentValue) {
            if (err) return callback(err, previousValue);
        ...
        function reduceAll() {
          source.next(function handler(err, currentValue) {
            if (err) {
              ...
              if (err == STOP) return callback(null, previousValue);
              else return err;
            }
    ...
    this.forEach = this.each = function (fn) {
      fn = makeAsync(1, fn);
      var source = this.source;
      this.next = function (callback) {
        source.next(function handler(err, value) {
        ...
        source.next(function handler(err, value) {
          ...
          condition(value, function (err, result) {
            if (err) {
              ...
              if (err == STOP) callback(null, false);
              else callback(err);
            } else if (result) {
              done = true;
              callback(null, true);
        ...
        source.next(function handler(err, value) {
          ...
          condition(value, function (err, result) {
            if (err) {
              ...
              if (err == STOP) callback(null, true);
              else callback(err);
            } else if (!result) {
              done = true;
              callback(null, false);
        ...
        source.next(function handler(err, value) {
          if (err) {
            if (err == STOP) {
              ...
              if (!source) return callback(STOP);
              else return source.next(handler);
            } else return callback(err);
          } else return callback(null, value);
        });
      });
    };
    ...
    this.sort = function (compare) {
      ...
      var arrGen;
      this.next = function (callback) {
        if (arrGen) return arrGen.next(callback);
    ...
    this.$arrayOp = function (arrayMethod, args) {
      ...
      var i = 0;
      this.next = function (callback) {
        if (i++ > 0) return callback(STOP);
        self.toArray(function (err, arr) {
          if (err) callback(err, "");
          else {
            if (args) callback(null, arrayMethod.apply(arr, args));
            else callback(null, arrayMethod.call(arr));
          }
        });
      };
      return new this.constructor(this);
    ...
    this.end = function (breakOnError, callback) {
      ...
      source.next(function handler(err, value) {
        if (err) {
          if (err == STOP) callback && callback(lastError, last);
          else if (!breakOnError) {
            lastError = err;
            source.next(handler);
          } else callback && callback(err, value);
        } else {
          last = value;
          source.next(handler);
        }
    ...
    this.toArray = function (breakOnError, callback) {
      ...
      source.next(function handler(err, value) {
        if (err) {
          if (err == STOP) {
            if (breakOnError) return callback(null, values);
            else {
              errors.length = values.length;
              return callback(errors, values);
            }
          } else {
            if (breakOnError) return callback(err);
            else errors[values.length] = err;
          }
        }
        values.push(value);
        source.next(handler);
  ...
  exports.range = function (start, stop, step, construct) {
    ...
    start = start || 0;
    step = step || 1;
    if (stop === undefined || stop === null) stop = step > 0 ? Infinity : -Infinity;
    var value = start;
    return new construct(function (callback) {
      if ((step > 0 && value >= stop) || (step < 0 && value <= stop)) callback(STOP);
      else {
        var current = value;
        value += step;
  ...
  exports.concat = function (first, varargs) {
    if (arguments.length > 1) return first.concat.apply(first, Array.prototype.slice.call(arguments, 1));
    else return first;
  };
  exports.zip = function (first, varargs) {
    if (arguments.length > 1) return first.zip.apply(first, Array.prototype.slice.call(arguments, 1));
    else
...
define("kitchen-sink/dev_util", [
  ...
  "ace/range",
  "ace/editor",
  "ace/test/asyncjs/assert",
  "ace/test/asyncjs/async",
  "ace/undomanager",
  "ace/edit_session",
  ...
  def(window, "editor", function () {
    return window.env.editor == logEditor ? editor : window.env.editor;
  });
  def(window, "session", function () {
    return window.editor.session;
  });
  def(window, "split", function () {
    return window.env.split;
  });

  def(window, "devUtil", function () {
    return exports;
  });
  ...
  function getSelection(editor) {
    ...
    data = data.map(function (x) {
      ...
      return Range.comparePoints(a, c) ? [a.row, a.column, c.row, c.column] : [a.row, a.column];
    });
  ...
  function toString(x) {
    ...
    switch (x.type) {
      case "exec":
        str = 'editor.execCommand("' + data.command.name + (data.args ? '", ' + JSON.stringify(data.args) : '"') + ")";
        break;
      case "setSelection":
  ...
  exports.textInputDebugger = {
    ...
    onchange: function (value) {
      ...
      if (env.textarea) {
        if (env.textarea.detach) env.textarea.detach();
    ...
    showConsole: function () {
      ...
      var events = ["select", "input", "keypress", "keydown", "keyup", "compositionstart", "compositionupdate", "compositionend", "cut", "copy", "paste"];
      events.forEach(function (name) {
        text.addEventListener(name, addToLog, true);
      });
      function onMousedown(ev) {
        if (ev.domEvent.target == text) ev.$pos = editor.getCursorPosition();
      }
  ...
  exports.openConsole = function () {
    ...
    if (!exports.session) exports.session = new EditSession("");
...
define("kitchen-sink/doclist", ["require", "exports", "module", "ace/edit_session", "ace/undomanager", "ace/lib/net", "ace/ext/modelist"], function (require, exports, module) {
  ...
  function initDoc(file, path, doc) {
    if (doc.prepare) file = doc.prepare(file);
  ...
  if (window.require && window.require.s)
    try {
      for (var path in window.require.s.contexts._.defined) {
        if (path.indexOf("!") != -1) path = path.split("!").pop();
        else path = path + ".js";
        ownSource[path] = "";
      }
    } catch (e) {}
  ...
  function prepareDocList(docs) {
    ...
    for (var path in docs) {
      var doc = docs[path];
      if (typeof doc != "object") doc = { name: doc || path };

      doc.path = path;
      doc.desc = doc.name.replace(/^(ace|docs|demo|build)\//, "");
      if (doc.desc.length > 18) doc.desc = doc.desc.slice(0, 7) + ".." + doc.desc.slice(-9);
  ...
  function loadDoc(name, callback) {
    ...
    if (doc.session) return callback(doc.session);
    var path = doc.path;
    var parts = path.split("/");
    if (parts[0] == "docs") path = "demo/kitchen-sink/" + path;
    else if (parts[0] == "ace") path = "lib/" + path;
  ...
  function saveDoc(name, callback) {
    ...
    if (!doc || !doc.session) return callback("Unknown document: " + name);
  ...
  function upload(url, data, callback) {
    ...
    if (/^file:/.test(absUrl)) absUrl = "http://localhost:8888/" + url;
    url = absUrl;
    if (!/^https?:/.test(url)) return callback(new Error("Unsupported url scheme"));
...
define("kitchen-sink/util", [
  ...
  exports.getOption = function (name) {
    if (urlOptions[name]) return urlOptions[name];
  ...
  function saveOptionFromElement(el, val) {
    if (!el.onchange && !el.onclick) return;
  ...
  exports.bindCheckbox = function (id, callback, noInit) {
    ...
    if (urlOptions[id]) el.checked = urlOptions[id] == "1";
    else if (localStorage && localStorage.getItem(id)) el.checked = localStorage.getItem(id) == "1";
  ...
  exports.bindDropdown = function (id, callback, noInit) {
    ...
    if (urlOptions[id]) el.value = urlOptions[id];
    else if (localStorage && localStorage.getItem(id)) el.value = localStorage.getItem(id);
  ...
  function optgroup(values) {
    return values.map(function (item) {
      if (typeof item == "string") item = { name: item, caption: item };
      return elt("option", { value: item.value || item.name }, item.caption || item.desc);
    });
  }

  function dropdown(values) {
    if (Array.isArray(values)) return optgroup(values);
...
define("ace/commands/incremental_search_commands", ["require", "exports", "module", "ace/config", "ace/lib/oop", "ace/keyboard/hash_handler", "ace/commands/occur_commands"], function (
  require,
  exports,
  module
) {
...
define("ace/split", ["require", "exports", "module", "ace/lib/oop", "ace/lib/lang", "ace/lib/event_emitter", "ace/editor", "ace/virtual_renderer", "ace/edit_session"], function (
  require,
  exports,
  module
) {
...
define("ace/ext/menu_tools/settings_menu.css", ["require", "exports", "module"], function (require, exports, module) {
  module.exports =
...
define("ace/ext/menu_tools/overlay_page", ["require", "exports", "module", "ace/lib/dom", "ace/ext/menu_tools/settings_menu.css"], function (require, exports, module) {
...
define("ace/ext/options", [
  ...
  (function () {
    ...
    this.render = function () {
      ...
      buildDom(
        [
          ...
          ["tr", null, ["td", { colspan: 2 }, ["table", { role: "presentation", id: "more-controls" }, this.renderOptionGroup(optionGroups.More)]]],
    ...
    this.renderOption = function (key, option) {
      ...
      return ["tr", { class: "ace_optionsMenuEntry" }, ["td", ["label", { for: safeKey, id: safeId }, key]], ["td", control]];
    };
...
define("ace/autocomplete/popup", ["require", "exports", "module", "ace/virtual_renderer", "ace/editor", "ace/range", "ace/lib/event", "ace/lib/lang", "ace/lib/dom"], function (
  require,
  exports,
  module
) {
...
define("ace/autocomplete", [
  ...
  (function () {
    ...
    this.openPopup = function (editor, prefix, keepPopupPosition) {
      ...
      if (this.editor.textInput.setAriaOptions) this.editor.textInput.setAriaOptions({ activeDescendant: getAriaId(this.popup.getRow()) });
      editor.keyBinding.addKeyboardHandler(this.keyboardHandler);
...
define("kitchen-sink/demo", [
  ...
  "kitchen-sink/dev_util",
  "kitchen-sink/file_drop",
  "ace/config",
  "ace/lib/dom",
  "ace/lib/net",
  "ace/lib/lang",
...
border:1px solid #baf; z-index:100";
  ...
  env.editor.showCommandLine = function (val) {
    this.cmdLine.focus();
    if (typeof val == "string") this.cmdLine.setValue(val, 1);
  };
  env.editor.commands.addCommands([
    {
      ...
      exec: function (editor, needle) {
        ...
        if (s) snippetManager.insertSnippet(editor, s.content);
      },
      readOnly: true,
    },
    {
  ...
  commands.addCommand({
    ...
    exec: function (arg) {
      ...
      localStorage.setItem("saved_file:" + name, session.getValue());
  ...
  function handleToggleActivate(target) {
    if (dom.hasCssClass(sidePanelContainer, "closed")) onResize(null, false);
    else if (dom.hasCssClass(target, "toggleButton")) onResize(null, true);
  }
  ...
  function onResize(e, closeSidePanel) {
    ...
    if (closeSidePanel == null) closeSidePanel = width < 2 * left;
    if (closeSidePanel) {
      left = 20;
      document.getElementById("optionToggle").setAttribute("aria-label", "Show Options");
    } else document.getElementById("optionToggle").setAttribute("aria-label", "Hide Options");
    width -= left;
  ...
  doclist.cycleOpen = function (editor, dir) {
    var h = this.history;
    h.index += dir;
    if (h.index >= h.length) h.index = 0;
    else if (h.index <= 0) h.index = h.length - 1;
    var s = h[h.index];
  ...
  optionsPanel.add({
    Main: {
      ...
      Split: {
        ...
        getValue: function () {
          var sp = env.split;
          return sp.getSplits() == 1 ? "None" : sp.getOrientation() == sp.BELOW ? "Below" : "Beside";
        },
      },
    },
    More: {
  ...
  for (var i in optionsPanel.options) {
    ...
    if (value != undefined) {
      if ((i == "mode" || i == "theme") && !/[/]/.test(value)) value = "ace/" + i + "/" + value;
  ...
  optionsPanelContainer.insertBefore(
    dom.buildDom([
      ...
      ["div", {}, ["button", { onclick: env.editSnippets }, "Edit Snippets"]],
      [
        "div",
        {},
        [
          "button",
          {
            onclick: function () {
              ...
              if (env.editor.getValue() == info) return env.editor.undo();
  ...
  event.addListener(document.documentElement, "keyup", function (e) {
    if (e.keyCode === 18)
      // do not trigger browser menu on windows
      e.preventDefault();
  });
  ...
  function moveFocus() {
    var el = document.activeElement;
    if (el == env.editor.textInput.getElement()) env.editor.cmdLine.focus();
    else env.editor.focus();
  }
});
(function () {
